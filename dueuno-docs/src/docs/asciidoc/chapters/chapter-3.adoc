...
To register a feature, we need to specify the name of the controller that implements it. For example, in our case, the feature will be available at the following URL:
...
. We use a convention to name actions. When they start with the `on` prefix, they execute logic in the background. When they don’t, they render a user interface. We are also using a naming standard here. We may change the action names, but for now let’s not add too many complications.
