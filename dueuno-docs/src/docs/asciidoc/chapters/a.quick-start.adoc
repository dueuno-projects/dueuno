...
=== Goals
*With Dueuno, you can develop business web applications by writing code in a single programming language: https://groovy-lang.org/[Apache Groovy,window=_blank]. There is no need to know HTML, CSS, or JavaScript.*

With Dueuno you can develop and maintain web applications with requirements such as:
...
NOTE: The main purpose of Dueuno is to decouple business logic from the _User Interface (GUI)_. This lowers application maintenance costs: the team uses just one language instead of five (HTML, CSS, JavaScript, Java, and SQL), developers need fewer specialized skills, and the GUI does not need to be upgraded whenever new web technologies or standards become available.

=== Non-Goals
*Dueuno is NOT a solution for:*
...
NOTE: Log in with `admin/admin` to manage the default tenant or with `super/super` to manage the whole application.

==== Apache Groovy & Grails
Dueuno applications are Grails applications so you need to get confident with the https://grails.org/[Apache Grails,window=_blank] framework and the https://groovy-lang.org/[Apache Groovy,window=_blank] programming language.
...
== Basics
In Dueuno everything is a component. All visual objects of the application are derived from the same base class `Component` and they can be assembled together like we do with a LEGO set.
...
=== The Shell
The `Shell` component is the Dueuno GUI. Each Dueuno application share a common user experience and content structure.
...
Modals can be displayed in three sizes: `normal` (default), `small`, `large` and `fullscreen`.
...
==== Responsiveness
All Dueuno applications work both on desktop computers and on mobile devices by design and without the developer having to cope with it. Here is how an application looks like on a Desktop, on a Tablet and on a Mobile Phone.
...
=== Project Structure
Dueuno applications are Grails applications. The project structure, follows the _conventions over configuration_ design paradigm so each folder contains a specific source file type.
...
A Dueuno application is a container for a set of _Features_.
...
<3> Within the `applicationService.onInit { ... }` closure, you can call any `applicationService` method. In this case, the method is xref:z.api-reference.adoc#registerFeature[`registerFeature()`].
...
Each application has a `DEFAULT` database connection defined in the `grails-app/conf/application.yml` file. This `DEFAULT` connection cannot be changed at runtime and it is used by Dueuno to store its own database.
...
. You can edit/create database connections at runtime from the Dueuno GUI accessing with the `super` user from the menu `System Configuration -> Connection Sources`
...
New Tenants can be created from the Dueuno GUI accessing as `super` from the menu `System Configuration -> Tenants`. If multi-tenancy is not a requirement to your application you will be using the `DEFAULT` Tenant which is automatically created.

=== User Management

Users can access a Dueuno application with credentials made of a username and a secret password. Each user must be configured by the Tenant's `admin` user from the menu `Tenant Administration -> Users` and `Tenant Administration -> Groups`.
...
Applications are made of features. We register one to work with movies (see xref:z.api-reference.adoc#features[Features]).
...
----
<1> See xref:z.api-reference.adoc#application[Application].
<2> See xref:z.api-reference.adoc#registerFeature[`registerFeature()`].
...
We are going to use `ContentTable` to list records, and `ContentCreate` and `ContentEdit` to create and edit them (see xref:z.api-reference.adoc#contents[Contents]).
...
Read xref:dueuno-elements-book[The Dueuno Book] or xref:api-reference[Dueuno API Reference].
endif::[]
ifeval::["{docname}" != "index"]
Read xref:chapter-1.adoc[The Dueuno Book] or xref:z.api-reference.adoc[Dueuno API Reference].
endif::[]
